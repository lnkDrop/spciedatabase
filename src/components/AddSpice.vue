<template>
	<div id="AddSpice">
		...
		<p>英文名称</p>
		<div>
			<el-input size="mini" v-model="englilsh_name"></el-input>
		</div>
		<p>产地</p>
		<div>		
		...
		<div id="add-main-element">
			...
			<ul id="add-main-element-col">
				<li class="manage-mainelement-list" v-for="(item,index) in phypors">
					...
					<div v-if="item.constant!==''" class="manage-mainelement-list-item">常数：<span>{{item.constant}}</span></div>
					<div v-if="item.describe!==''" class="manage-mainelement-list-item">描述：<span>{{item.describe}}</span></div>
					<div v-if="item.minvalue!==''" class="manage-mainelement-list-item">最小值：<span>{{item.minvalue}}</span></div>
					<div v-if="item.maxvalue!==''" class="manage-mainelement-list-item">最大值：<span>{{item.maxvalue}}</span></div>
					<div class="manage-mainelement-list-item">
		...
		<div id="spice-do-some">
			<el-button @click="AddSpice()" type="primary">提交</el-button>
				...
				if(this.value1===""||this.value2===""||this.value3===""||this.name===""){
					...
					this.$axios({
						...
						data:{
							...
							"english_name":this.englilsh_name,
						...
						if(res.data === 1){
							console.log('')
							this.$message.success('添加香料信息成功')
							this.$router.push('/home/managespice')
						}else{
							this.$message.error('添加香料信息失败')
							this.$router.push('/home/managespice')
						}
					})
				}
